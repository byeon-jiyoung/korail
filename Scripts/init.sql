...
CREATE TABLE korail.member (
	...
	mem_mileage    INT         NULL     DEFAULT 0 COMMENT '마일리지', -- 마일리지
...
CREATE TABLE korail.reservation (
	...
	nomem_phone VARCHAR(50) NULL     COMMENT '비회원전화번호' -- 비회원전화번호
)
COMMENT '예매';
...
CREATE TABLE korail.sale (
	...
	sal_classify TINYINT NULL     COMMENT '0은 카드 & 1은 계좌이', -- 결제분류
	sal_cancel   TINYINT NULL     DEFAULT false COMMENT '결제취소여부' -- 결제취소여부
)
COMMENT '결제';
